...
(use-package vterm
  ...
  (defun my/short-dir (dir)
    (let ((parts (split-string (abbreviate-file-name dir) "/" t)))
      (concat (mapconcat #'identity (last parts 2) "/") "/")))
  (advice-add 'vterm :before
    (lambda (&rest _)
      (setq vterm-buffer-name (concat "*" (my/short-dir default-directory) "*"))))
  ...
  (add-hook 'vterm-set-directory-functions
            (lambda (dir)
              ...
              (rename-buffer (concat "*" (my/short-dir dir) "*") t)))
