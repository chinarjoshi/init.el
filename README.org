...
#+begin_src emacs-lisp
(defun my/set-fonts ()
  (set-face-attribute 'default nil :family "Inconsolata Nerd Font" :height 140)
  (set-face-attribute 'fixed-pitch nil :family "Inconsolata Nerd Font" :height 140)
...
** Clipboard
Wayland integration via wl-copy/wl-paste.
...
(defun notes--add-top-padding ()
  "Add negative space above title line."
  (when (derived-mode-p 'org-mode)
    (save-excursion
      (goto-char (point-min))
      (when (re-search-forward "^#\\+title:" nil t)
        (let ((ov (make-overlay (line-beginning-position) (line-beginning-position))))
          (overlay-put ov 'before-string (propertize " " 'display '(space :height 3))))))))

(defun notes--start-cursor-after-title ()
  "Start cursor on line 3 after title and blank line."
  (goto-char (point-min))
  (forward-line 2))

(add-hook 'org-mode-hook #'notes--add-top-padding)
(add-hook 'org-mode-hook #'notes--start-cursor-after-title)
(custom-set-faces
 ...
 '(org-document-title ((t (:height 1.5 :weight bold :foreground "#e06c75"))))
 '(org-level-1 ((t (:height 1.3 :weight bold)))))
#+end_src

** Notes
Obsidian-style periodic notes (daily/weekly/monthly).
...
(defun notes--insert-title (file)
  "Insert title for notes if buffer is empty (Obsidian-style)."
  (when (= (buffer-size) 0)
...
(defun notes-open-daily ()
  ...
  (let* ((file (notes--daily-file (current-time)))
    ...
    (find-file file)
    (when new-file (notes--insert-title file))))
...
** Pieces
Act/Piece file management with tagging and capture.
...
** Transclusions
Auto-generates and manages transclusion links for periodic notes.
...
** Leader Key
Helix-style space mode with hidden piece bindings (1-9 for acts, 1-9 for pieces).
...
** Languages
Configures Eglot for various languages (Go, Python, Shell, Web, C/C++, Lua, Nix).
