...
(defun notes--add-virtual-title ()
  "Add virtual title overlay at top of buffer showing filename."
  (when buffer-file-name
    (let* ((title (file-name-sans-extension (file-name-nondirectory buffer-file-name)))
           (ov (make-overlay 1 1))
           (display-text (propertize
                         (concat "\n" title "\n\n")
                         'face '(:height 1.5 :weight bold :foreground "#e06c75" :family "Inter"))))
      (overlay-put ov 'before-string display-text))))

(defun notes--cursor-after-virtual-title ()
  "Move cursor below virtual title."
  (goto-char (point-min)))

(add-hook 'org-mode-hook #'notes--add-virtual-title)
(add-hook 'markdown-mode-hook #'notes--add-virtual-title)
(add-hook 'org-mode-hook #'notes--cursor-after-virtual-title)
(add-hook 'markdown-mode-hook #'notes--cursor-after-virtual-title)
(custom-set-faces
