...
;; Keybindings set after Evil loads (see Evil section)
#+end_src
...
(use-package org-transclusion
  ...
  :after org
  :config
  ;; Make C-c C-c work in Evil insert state for live-sync
  (evil-define-key 'insert org-transclusion-live-sync-map
    (kbd "C-c C-c") #'org-transclusion-live-sync-exit)

  ;; Wrap transclusions in heading based on source filename
               ...
               (let ((name (file-name-sans-extension
                 ...
                 (with-temp-buffer
                   ...
                   (while (re-search-forward "^\\(\\*+\\) " nil t)
                     (replace-match "*\\1 "))
                   ;; Add wrapper heading at the start
                   (goto-char (point-min))
                   (insert (format "* %s\n" name))
                   (buffer-string)))
             content)))
...
(defun notes--update-transclusions ()
  ...
  (when buffer-file-name
    ;; Ensure title exists before adding transclusions
    (when (= (buffer-size) 0)
      (let ((name (file-name-sans-extension (file-name-nondirectory buffer-file-name))))
        (insert (format "#+title: %s\n\n" name))
        (save-buffer)))
...
(use-package evil
  ...
  ;; Notes bindings (override Evil's | column motion)
  (evil-define-key 'normal 'global
    "|" 'notes-open-daily)
